...
const MyList = () => {
  ...
  return (
    <>
      <Link to="/user/main" className="my-item">
        我的主页
      </Link>
      <p className="my-item" onClick={logout}>
        退出
      </p>
    </>
  );
...
const Header = (props) => {
  ...
  // 第二个参数是空的，只会在组件挂载和卸载的时候调用
  useEffect(() => {
    searchRef.current.value = decodeURIComponent(kw);
    if (!kw) return;
    try {
      document.querySelector(".srch.js-flag").value = kw;
      setTimeout(() => {
        document.querySelector(".btn.js-flag").click();
      }, 10);
    } catch (error) {
      console.log(error);
    }
        ...
        <div className="wrap f-cb">
          ...
          <div className="m-tophead f-pr j-tflag">
            {user.token ? (
              ...
              <div className="link s-fc3" onClick={goLogin}>
                登录
              </div>
            )}
          </div>
