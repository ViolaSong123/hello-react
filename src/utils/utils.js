...
export function mediaTimeFormat(time) {
  //毫秒转
...
export function artistsFormat(ar, attr = "name") {
  //多位艺人合并
  let artistsArr = [];
  ar.map((item) => {
...
export function playTimesFormat(num, unit = 10000, unitName = "万") {
  //数量级转换
  return num >= unit ? (num / unit).toFixed(1) + unitName : num;
}

export function getRandom(min, max, exclude = []) {
  //产生随机数
...
export function playItem(id) {
  ...
  let exist = false;
  const curSong = store.getState().globalData.curSong || {};
  for (let i = 0; i < historyPlay.length; i++) {
    if (historyPlay[i].id === id) {
      if (curSong.id && id === curSong.id) return;
      exist = true;
      break;
    }
  }
...
export async function delFromPlay(id, e) {
  //从播放列表里删除
  e.stopPropagation();
  const historyPlay = store.getState().globalData.historyPlay;
  let idx, curId;
  ...
  if (curId === store.getState().globalData.curSong.id) {
    //删除的正在playing
...
export async function playList(id, songs = []) {
  //播放整张歌单
  let res = songs;
  if (songs.length === 0) {
