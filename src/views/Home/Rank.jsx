  ...
  return (
    <li>
      ...
      <div className="oper">
        <em
          title="播放"
          className="s-bg s-bg-11"
          onClick={() => playItem(item.id)}
        ></em>
        <em
          title="添加到播放列表"
          className="u-icn u-icn-81"
          onClick={() => addToPlay(item.id)}
        ></em>
  ...
  return (
    <div className="n-bilst">
      {ranksData.map((rank, x) => {
        return (
          <dl
            className={["blk", x === ranksData.length - 1 ? "blk-1" : ""].join(
              " "
            )}
            key={rank.self.id}
          >
            <dt className="top">
              <div className="cver u-cover u-cover-4">
                <img
                  src={rank.self.coverImgUrl + "param=100y100"}
                  className="j-img"
                  alt=""
                />
                <Link
                  to={`/home/toplist/${rank.self.id}`}
                  className="msk"
                ></Link>
              </div>
              <div className="tit">
                ...
                <div className="btn">
                  <span
                    className="s-bg s-bg-9 f-tdn"
                    onClick={() => playList(rank.self.id)}
                  >
                    播放
                  </span>
            ...
            <dd>
              <ol>
                {rank.subs.map((item, i) => {
                  return <LiItem key={item.id} item={item} idx={i} />;
                })}
              </ol>
            </dd>
          </dl>
